 ...
 */
final class Task extends RecursiveAction implements SerializableRunnable, KryoSerializable {
	...
	/**
	 * Extra Finish instances in which this instance is registered.
	 * <p>
	 * This array will be of size 0 under the "normal" finish/async model. Only when
	 * the
	 * {@link apgas.ExtendedConstructs#asyncAtWithCoFinish(apgas.Place, SerializableJob, Finish...)}
	 * is called will this array be populated.
	 */
	Finish[] coFinish;
	 ...
	 */
	Task(Finish finish, Job f, int parent, Finish... extraFinishes) {
		this.finish = finish;
		this.f = f;
		this.parent = parent;
		coFinish = extraFinishes;
	}
	 ...
	 *
	 * @param worker the worker thread running the task or null if not a worker
	 *               thread
	 */
	void finish(Worker worker) {
		if (worker == null) {
			...
			compute();
			Task t;
			while (!finish.isReleasable() && (t = (Task) ForkJoinTask.peekNextLocalTask()) != null && finish == t.finish
					&& t.tryUnfork()) {
				t.compute();
			}
			try {
	...
	void asyncAt(int p) {
		try {
			...
			finish.unspawn(p);
			if (GlobalRuntimeImpl.getRuntime().verboseSerialization && !(e instanceof DeadPlaceException)) {
	...
	private void writeObject(ObjectOutputStream out) throws IOException {
		...
		out.writeInt(coFinish.length);
		for (Finish coF : coFinish) {
			out.writeObject(coF);
		}
	}

	public static final SerializableJob NULL = () -> {
	};
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
		...
		try {
			...
			if (GlobalRuntimeImpl.getRuntime().verboseSerialization && !(e instanceof DeadPlaceException)) {
				System.err.println("[APGAS] Failed to receive a task at place " + GlobalRuntimeImpl.getRuntime().here
						+ " due to: " + e);
				e.printStackTrace();
			}
		...
		int coFinishCount = in.readInt();
		coFinish = new Finish[coFinishCount];
		for (int i = 0; i < coFinishCount; i++) {
			coFinish[i] = (Finish) in.readObject();
		}
	}
	...
	public void write(Kryo kryo, Output output) {
		...
		output.write(coFinish.length);
		for (Finish coF : coFinish) {
			kryo.writeClassAndObject(output, coF);
		}
	}
	...
	public void read(Kryo kryo, Input input) {
		...
		int coFinishCount = input.read();
		coFinish = new Finish[coFinishCount];
		for (int i = 0; i < coFinishCount; i++) {
			coFinish[i] = (Finish) kryo.readClassAndObject(input);
		}
	}
}
