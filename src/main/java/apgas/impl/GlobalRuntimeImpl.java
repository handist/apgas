...
public final class GlobalRuntimeImpl extends GlobalRuntime {
	...
	public GlobalRuntimeImpl(String[] args) {
		try {
			...
			final String serialization = System.getProperty(Config.APGAS_SERIALIZATION, "kryo");
				...
				if (launcherName != null) {
					try {
						launcher = (Launcher) Class.forName(launcherName).newInstance();
							...
							| ClassNotFoundException | NoClassDefFoundError | ClassCastException e) {
						System.err.println("[APGAS] Unable to instantiate launcher: " + launcherName
			...
			if (finishName != null) {
				...
				try {
					factory = (Finish.Factory) Class.forName(finishFactoryName).newInstance();
				} catch (InstantiationException | IllegalAccessException | ExceptionInInitializerError
						| ClassNotFoundException | NoClassDefFoundError | ClassCastException e) {
					System.err.println("[APGAS] Unable to instantiate finish factory: " + finishFactoryName
			...
			if (kryo && !"kryo".equals(serialization)) {
				System.err.println("[APGAS] Unable to instantiate serialization framework: " + serialization
			...
			if (transportName != null) {
				try {
					transport = (Transport) Class.forName(transportName)
							.getDeclaredConstructor(GlobalRuntimeImpl.class, String.class, String.class, boolean.class)
							.newInstance(this, master, ip, compact);
				} catch (InstantiationException | IllegalAccessException | ExceptionInInitializerError
						| ClassNotFoundException | NoClassDefFoundError | ClassCastException e) {
					System.err.println(
							"[APGAS] Unable to instantiate transport: " + transportName + ". Using default transport.");
				}
			}
			if (transport == null) {
			...
			if (verboseLauncher) {
				System.err.println("[APGAS] New place starting at " + transport.getAddress() + ".");
			}
			...
			resilientFinishMap = resilient ? transport.<GlobalID, ResilientFinishState>getResilientFinishMap() : null;
			...
			if (master == null && p > 1) {
				try {
					...
					command.add("-D" + Configuration.APGAS_MASTER + "=" + transport.getAddress());
	 ...
	 * with body {@code f} and returns immediately.
	 *
	...
	/**
	 * Returns the local {@link Finish} instance in which the current activity is
	 * participating
	 * 
	 * @return current {@link Finish}, or null if not called from a {@link Worker}
	 */
	public Finish currentFinish() {
		Worker w = currentWorker();
		return w == null ? null : w.task.finish;
	}

	/**
	 * Implementation of
	 * {@link apgas.ExtendedConstructs#asyncAtWithCoFinish(Place, SerializableJob, Finish...)}
	 * 
	 * @param p        destination place for the remote job
	 * @param j        job to execute
	 * @param coFinish extra finish in which this job will be registered along with
	 *                 the current job of the calling thread
	 */
	public void asyncAtWithCoFinish(Place p, SerializableJob j, Finish[] coFinish) {
		final Worker worker = currentWorker();
		final Finish finish = worker == null || worker.task == null ? NullFinish.SINGLETON : worker.task.finish;
		finish.spawn(p.id);
		for (Finish coF : coFinish) {
			coF.spawn(p.id);
		}
	...
	/**
	 * Implementation of
	 * {@link apgas.ExtendedConstructs#asyncDifferentFinish(Finish, Job)}
	 * 
	 * @param f finish under which the asynchronous task is to be submitted
	 * @param j job to perform synchronously
	 */
	public void asyncDifferentFinish(Finish f, Job j) {
		f.spawn(here);
		new Task(f, j, here).async(null);
	}
}
