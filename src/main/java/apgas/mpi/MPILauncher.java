 ...
 * {@link MPILauncher#registerPlugins(Plugin)} before calling this class's main
...
final public class MPILauncher implements Launcher {
	...
	public static void registerPlugin(Plugin plugin) {
		if(verboseLauncher) {
			System.err.println("[MPILauncher] " + plugin.getName() + 
					" registered.");
		}
		for (Plugin p : plugins) {
			if (p.getName().equals(plugin.getName())) {
				System.err.println("[Warning][MPILauncher#registerPlugin] The plugin " + 
						plugin.getName() + " was previously registered," + 
						" skipping duplicate.");
			}
		}
		plugins.add(plugin);
